...
def run(project_dir: Path, state: dict) -> dict:
    ...
    external_justifications = (
        _justify_external_sources(external_sources, strategy, topic)
    ...
    if data_selection["use_external"]:
        ...
        assessments = _fix_known_source_assessments(
            assessments, external_sources, project_dir,
        )
...
# ── Known Peru government data source catalog ────────────────────────────────
# Maps canonical source names → verified URLs and download instructions.
# Used in run() to fix incorrect URLs generated by Claude in
# _assess_merge_feasibility(). Each entry has:
#   keywords     — list of lowercase terms to match against source names
#   url          — verified portal URL
#   wfs_url      — WFS endpoint (for INGEMMET auto-download)
#   instructions — manual download steps
#   auto_download — True if _try_download_ingemmet can handle this source

_KNOWN_PERU_SOURCES: list[dict] = [
    {
        "keywords": ["mef", "consulta amigable", "canon minero", "canon y sobrecanon",
                     "transferencias", "regalias", "regalías", "canon mining",
                     "district-level canon", "canon transfers"],
        "url": "https://apps5.mineco.gob.pe/transparencia/Navegador/default.aspx",
        "wfs_url": "",
        "name": "MEF Consulta Amigable — Canon Minero y Transferencias",
        "instructions": (
            ...
            "6. Clic en 'Consultar' y luego 'Exportar' -> CSV/Excel\n"
            "7. Guardar en data/external/mef/canon_minero_YYYY.csv"
        ),
        "auto_download": False,
    },
    {
        "keywords": ["ingemmet", "geocatmin", "geological mineral", "mineral deposit",
                     "deposito mineral", "depósito mineral", "catastro minero",
                     "geologia", "geología", "mining cadastre", "geological map"],
        "url": "https://geocatmin.ingemmet.gob.pe/",
        "wfs_url": "http://geocatmin.ingemmet.gob.pe/arcgis/services/SERV_GEOLOGIA_M/MapServer/WFSServer?",
        "name": "INGEMMET GEOCATMIN — Depósitos Minerales y Catastro Minero",
        "instructions": (
            "Descarga automática vía WFS o manual desde portal.\n"
            "WFS: http://geocatmin.ingemmet.gob.pe/arcgis/services/SERV_GEOLOGIA_M/MapServer/WFSServer?\n"
            "Capas: depósitos minerales, catastro minero, geología 1:1M.\n"
            "Usar QGIS (Añadir capa WFS) o Python con owslib/geopandas."
        ),
        "auto_download": True,  # _try_download_ingemmet can handle this
    },
    {
        "keywords": ["inei shapefile", "district boundary", "limite distrital",
                     "límite distrital", "inei boundary", "shapefile distrital",
                     "distrito shapefile", "administrative boundary peru"],
        "url": "https://ide.inei.gob.pe/",
        "wfs_url": "",
        "name": "INEI / GADM — Shapefiles de Límites Distritales",
        "instructions": (
            "Opcion A (oficial): https://ide.inei.gob.pe/ -> 'Descargas' -> 'Limites Politico-Administrativos' -> 'Distrital'\n"
            "Opcion B (GADM, descarga directa): https://gadm.org/download_country.html -> Peru -> Level 2 (distritos)\n"
            "Descargar shapefile (.shp + .dbf + .shx). Atributo UBIGEO para merge."
        ),
        "auto_download": False,
    },
    {
        "keywords": ["enaho", "encuesta nacional de hogares", "inei microdatos",
                     "inei microdata"],
        "url": "https://proyectos.inei.gob.pe/microdatos/",
        "wfs_url": "",
        "name": "INEI Microdatos — ENAHO",
        "instructions": (
            "Ya tienes ENAHO 2024 como data/enaho_2024_clean.csv.\n"
            "Si necesitas módulos adicionales: https://proyectos.inei.gob.pe/microdatos/\n"
            "-> 'Encuesta Nacional de Hogares' -> ano -> 'Bases de Datos' -> descargar .sav/.dta"
        ),
        "auto_download": False,
    },
    {
        "keywords": ["datos abiertos", "datosabiertos", "open data peru",
                     "plataforma nacional de datos abiertos"],
        "url": "https://www.datosabiertos.gob.pe/",
        "wfs_url": "",
        "name": "Plataforma Nacional de Datos Abiertos del Perú",
        "instructions": (
            "URL: https://www.datosabiertos.gob.pe/\n"
            "CKAN API: https://www.datosabiertos.gob.pe/api/3/\n"
            "Buscar datasets por keyword y descargar en CSV/JSON."
        ),
        "auto_download": False,
    },
]
...
def _match_known_peru_source(source_name: str) -> dict | None:
    """Return the known-source entry if source_name matches any keywords."""
    src_lower = source_name.lower()
    for entry in _KNOWN_PERU_SOURCES:
        if any(kw in src_lower for kw in entry["keywords"]):
            return entry
    return None


def _fix_known_source_assessments(
    assessments: list[dict],
    external_sources: list[str],
    project_dir: Path,
) -> list[dict]:
    """Post-process Claude's merge assessments to fix known-source URLs.

    Claude often hallucinates wrong URLs for Peruvian government sources.
    This function:
      1. Fixes download_url + download_instructions for matched known sources
      2. Attempts auto-download for sources with auto_download=True (INGEMMET)
      3. Returns corrected assessments with any auto-downloaded file paths
    """
    data_dir = project_dir / "data" / "external"
    data_dir.mkdir(parents=True, exist_ok=True)

    for assessment in assessments:
        source_name = assessment.get("source", "")
        known = _match_known_peru_source(source_name)

        if not known:
            continue

        print(f"\n  [fix] Known source detected: '{source_name[:60]}'")
        print(f"  [fix] Using verified URL: {known['url']}")

        # Fix the URL and instructions
        assessment["download_url"] = known["url"]
        if known.get("wfs_url"):
            assessment["wfs_url"] = known["wfs_url"]
        assessment["download_instructions"] = known["instructions"]

        # Attempt auto-download
        if known.get("auto_download"):
            print(f"  [fix] Attempting auto-download via WFS...")
            try:
                from .stage1_5_data_loading import _try_download_ingemmet

                # Build a minimal dataset_info dict for the downloader
                ds_info = {
                    "name": known["name"],
                    "download_url": known.get("wfs_url", known["url"]),
                    "source_api": "ingemmet",
                }
                local_path = _try_download_ingemmet(ds_info, data_dir)
                if local_path:
                    print(f"  [fix] Auto-downloaded: {local_path}")
                    assessment["local_path"] = local_path
                    assessment["auto_downloaded"] = True
                else:
                    print(f"  [fix] Auto-download failed — falling back to manual")
                    print(f"  [fix] Instructions:\n{known['instructions']}")
            except Exception as e:
                print(f"  [fix] Auto-download error: {e}")
        else:
            print(f"  [fix] Manual download required (no API)")
            print(f"  [fix] {'-' * 60}")
            for line in known["instructions"].split("\n"):
                print(f"  [fix] {line}")
            print(f"  [fix] {'-' * 60}")
...
# (End of known-source catalog)
    ...
    if data_selection["use_external"]:
        ...
        # ── Fix known-source URLs + attempt auto-download (INGEMMET WFS) ──
        assessments = _fix_known_source_assessments(
            assessments, external_sources, project_dir,
        )
